...
export async function createFolder(body: CreateFolderType) {
  ...
  return app.v1
    .spaces({ spaceId })
    .items.folders.post({ parentId, name } as any);
}
...
export type HardDeleteBatchItemsBody = {
  spaceId: string;
  itemIds: string[];
};

export async function hardDeleteBatchItems(params: HardDeleteBatchItemsBody) {
  const { spaceId, itemIds } = params;
  return app.v1
    .spaces({ spaceId })
    ["hard-delete-batch-items"].post({ itemIds } as any, {
      query: { spaceId },
    });
}
