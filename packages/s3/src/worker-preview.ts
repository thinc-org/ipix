...
const isImageContentType = (ct?: string | null) =>
	!!ct && /^(image)\/(jpeg|jpg|png|webp|avif|gif|tiff|bmp|x-icon|svg\+xml)$/i.test(ct);

function detectOutputFormat(ext?: string | null, fallbackCt?: string | null): {
	...
	if (ct.includes("jpeg") || ct.includes("jpg")) return { fmt: "jpeg", mime: "image/jpeg" };
...
async function getSourceObjectBufferAndCt(key: string): Promise<{ buf: Buffer; contentType: string | undefined }>
{
...
async function generatePreviewBuffer(
	...
	// Download source
	const { buf: src, contentType } = await getSourceObjectBufferAndCt(inputKey);
	...
	const maxEdge = (variant.maxEdge ?? (variant as any).max) as number | undefined;
...
async function copyOrPutPreview(
	...
	try {
		const processed = await generatePreviewBuffer(srcKey, variant, sourceCtHint);
		if (processed) {
			await s3.send(
...
export const previewWorker = new Worker<PreviewJob>(
	...
	for (const v of job.data.variants) {
			...
			// Write the preview
			await copyOrPutPreview(sourceKey, objectKey, { ...v, ext }, job.data.contentType);
		}
